...
make_metrics_plots <- function(file_name) {
  all_sims <- read_csv(file_name) %>% rename_with(~str_remove(., "prevalence_")) %>% select(-any_of(c("strains", "params")))
...
# multi strain main parameterization
# metrics_plots <- make_metrics_plots("../../Results/LNS_MANTIS_g0.66_s8.0/large-network-simulations.csv")
# {metrics_plots[[1]] + metrics_plots[[2]] + metrics_plots[[3]] +
#     # manually set axis.text.x for aesthetics
#     metrics_plots[[4]] + scale_x_log10(breaks=10^(-1:3), labels=c("0.1", "1", "10", "100", "1000")) +
#     plot_layout(nrow=1)} %T>%
#   ggsave(file="main_metrics.pdf", width=9, height=2.67, device=cairo_pdf)
# make_propdyn_plot("../../Results/LNS_MANTIS_g0.66_s8.0/large-network-simulations.csv") %T>%
#   ggsave(filename="main_propdyn.pdf", width=8, height=2.5, device=cairo_pdf)
#
# # multi strain alternate parameterization
# metrics_plots <- make_metrics_plots("../../Results/LNS_MANTIS_g0.55_s32.0/large-network-simulations.csv")
# {metrics_plots[[1]] + metrics_plots[[2]] + metrics_plots[[3]] + metrics_plots[[4]] + plot_layout(nrow=1)} %T>%
#   ggsave(file="alternate_metrics.pdf", width=9, height=2.67, device=cairo_pdf)
# make_propdyn_plot("../../Results/LNS_MANTIS_g0.55_s32.0/large-network-simulations.csv") %T>%
#   ggsave(filename="alternate_propdyn.pdf", width=8, height=2.5, device=cairo_pdf)

metrics_plots <- make_metrics_plots("../../Results/LNS_MANTIS_22/large-network-simulations.csv")
{metrics_plots[[1]] + metrics_plots[[2]] + metrics_plots[[3]] +
    # manually set axis.text.x for aesthetics
    metrics_plots[[4]] + scale_x_log10(breaks=10^(-1:3), labels=c("0.1", "1", "10", "100", "1000")) +
    plot_layout(nrow=1)} %T>%
  ggsave(file="main_metrics.pdf", width=9, height=2.67, device=cairo_pdf)
make_propdyn_plot("../../Results/LNS_MANTIS_22/large-network-simulations.csv") %T>%
  ggsave(filename="main_propdyn.pdf", width=8, height=2.5, device=cairo_pdf)

metrics_plots <- make_metrics_plots("../../Results/LNS_MANTIS_333/large-network-simulations.csv")
{metrics_plots[[1]] + metrics_plots[[2]] + metrics_plots[[3]] +
    # manually set axis.text.x for aesthetics
    metrics_plots[[4]] + scale_x_log10(breaks=10^(-1:3), labels=c("0.1", "1", "10", "100", "1000")) +
    plot_layout(nrow=1)} %T>%
  ggsave(file="alternate_metrics.pdf", width=9, height=2.67, device=cairo_pdf)
make_propdyn_plot("../../Results/LNS_MANTIS_333/large-network-simulations.csv") %T>%
  ggsave(filename="alternate_propdyn.pdf", width=8, height=2.5, device=cairo_pdf)

################################################################################

make_figs_old_data <- function(data_file, fig_id) {
  load(data_file)
  tmp <- all_sims %>%
...
metrics_plots <- make_figs_old_data("large-network-simulations_g0.66,s8,m0.05,d0.01.RData", "main")
ggsave(gridExtra::arrangeGrob(metrics_plots[[1]],
                              metrics_plots[[2]] +
                                theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()),
                              # + facet_zoom(xlim=c(0.85, 0.9), zoom.size=1),
                              metrics_plots[[3]] +
                                theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()),
                              # + facet_zoom(xlim = log10(c(6.5, 35)), zoom.size=1),
                              # filler_plot,
                              # layout_matrix=matrix(c(1,2,3,4,2,3), 2, 3, byrow=TRUE),
                              widths=c(1.3,1,1)), #heights=c(4,4)),
       file="main_metrics.pdf", width=9, height=3, device=cairo_pdf)

metrics_plots <- make_figs_old_data("large-network-simulations_g0.55,s32,m0.05,d0.01.RData", "alternate")
ggsave(gridExtra::arrangeGrob(metrics_plots[[1]],
                              metrics_plots[[2]] +
                                theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()),
                                # facet_zoom(xlim=c(0.8, 0.91), zoom.size=1),
                              metrics_plots[[3]] +
                                theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()),
                                # facet_zoom(xlim = log10(c(2.5, 25)), zoom.size=1),
                              # filler_plot,
                              # layout_matrix=matrix(c(1,2,3,4,2,3), 2, 3, byrow=TRUE),
                              widths=c(1.3,1,1)), #heights=c(4,4)),
